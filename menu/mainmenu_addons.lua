...
function PANEL:Init()
	...
	function self:UpdateAddons(search)
		...
		self.coroutines[#self.coroutines+1] = coro
	end

	local function RefreshAddons()
		timer.Create("menu_refreshaddons", 0.1, 1, function() self:UpdateAddons() end)
	end
...
--[[
	...
	for i, c in pairs(self.coroutines) do
		if c and coroutine.status(c) ~= "dead" then coroutine.resume(c) else self.coroutines[i] = nil end
	end
end
